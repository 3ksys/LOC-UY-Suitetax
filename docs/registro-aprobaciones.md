...
| STC-A2 | Seteo de Tax Codes | Manejo de error del `save`: hoy si falla, la transacción queda sin tax codes sin alerta | 🔴 Alto | ⏳ Pendiente | Mobeats · 2026-07-09 | — | ~~Se resuelve solo si se aprueba STC-A1~~ — **corregido 2026-08-20:** el híbrido conserva el `save` en la rama legacy, así que el hallazgo **sobrevive con alcance reducido** (solo el camino degradado). Sigue requiriendo aprobación: cualquier alerta o `throw` cambia comportamiento visible. Ver [propuesta STC-A2/A3](propuestas/STC-A2-A3-manejo-error-y-multiples-taxdetails.md) |
