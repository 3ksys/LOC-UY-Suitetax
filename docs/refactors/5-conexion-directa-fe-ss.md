...
Todas fuera del refactor; requieren decisión/aprobación de Tekiio.

> 📋 **Registrados (2026-09-15).** `CDF-A1`…`CDF-A8`, más los 🔴 `CDF-B1`, `CDF-B5` y `CDF-D1` del §3, tienen fila propia en [registro-aprobaciones.md](../registro-aprobaciones.md), todos en estado ⏳ Pendiente. **Ninguno se presentó formalmente a Tekiio todavía.** `CDF-D1` se registró aunque sea 💡 candidato fuera de esta ola, para que la aprobación no se pierda cuando llegue su turno.
...
## Anexo: flujo del tipo de CFE → [documento aparte](5-conexion-directa-fe-ss-anexo-tipo-cfe.md)

La traza completa de cómo el script resuelve el tipo de CFE — evidencia para el diagnóstico del **error 100000** — está en el [anexo](5-conexion-directa-fe-ss-anexo-tipo-cfe.md), para que este informe se lea rápido.
