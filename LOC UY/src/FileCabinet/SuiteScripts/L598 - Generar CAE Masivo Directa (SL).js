...
define(["N/log", "N/search", "N/runtime", "N/format", "N/ui/serverWidget", "N/task", "L598/utilities"],
  function (log, search, runtime, format, serverWidget, task, utilities) {
    ...
    function obtenerSubsidiarias(parametroPais){
      ...
      try {
        //var resultSubsidiaria = new Array();
        //nlapiLogExecution("DEBUG", "Procesar Transacciones", 'Inicio Obtener Subsidiarias');
        var i=0;
        ...
        if (!l598isEmpty(parametroPais)){
          ...
          filtros.push(filtro);
          //filtro[i++] = new nlobjSearchFilter('country', null, 'anyof', parametroPais);
        }
    ...
    function procesarTransacciones(context) {
      ...
      try {
        ...
        // Envio los Filtros
        params.custscript_l598_gen_cae_a_transacciones = JSON.stringify(objParams);
        ...
        mrTask.scriptId = "customscript_l598_gen_cae_aut_dir";
        //mrTask.deploymentId = "customdeploy_l598_generar_cae_lotes_v2";
        mrTask.params = params;
        var taskId = mrTask.submit(); //Submit MapReduce

        const status = taskId;

        log.debug("URU - SUITELET Generar CAE ", "STATUS ENCOLADO Script Map Reduce : " + status);
    ...
    function buscarTransacciones(context) {
      ...
      try {
        ...
        const searchSave = search.load({ id: "customsearch_l598_tran_ge_cae_con_dir_sl" });
        log.debug(proceso, `LINE 342 - filtros SS de tipo transaccion: ${JSON.stringify(searchSave)}`);
        searchSave.filters.push(...filtros);
        log.debug(proceso, `LINE 344 - filtros SS de tipo transaccion: ${JSON.stringify(searchSave)}`);
        ...
        log.debug(proceso, `LINE 350 - filtros SS de tipo transaccion: ${JSON.stringify(searchResults)}`);
        const completeResultSet = searchResults.getRange(0, 1000);
        log.debug(proceso, `LINE 354 - completeResultSet.length: ${completeResultSet.length}`);
        if (!l598isEmpty(completeResultSet) && completeResultSet.length > 0) {
          existenTransacciones = completeResultSet[0].getValue({ name: completeResultSet[0].columns[0] });
        } 
      ...
      log.audit("URU - SUITELET Generar CAE ", "FIN CONSULTA TRANSACCIONES ----- Existen Transacciones = " + existenTransacciones);
    ...
    function onRequest(context) {
      ...
      try {
        ...
        const form = serverWidget.createForm("Factura Electronica (Conexión Directa)");
        form.clientScriptModulePath = "./L598 - Generar CAE Masivo Directa (Cliente).js";
        ...
        if (oneWorld == true) {
          ...
          let paramPais = script.getParameter('custscript_l598_con_proces_trans_country');
