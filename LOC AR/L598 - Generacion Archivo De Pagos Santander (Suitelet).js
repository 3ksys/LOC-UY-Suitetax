...
define(['N/ui/serverWidget', 'N/https', 'N/record', 'N/error', 'N/search', 'N/format', 'N/task', '3K/utilities'],
    ...
    function(serverWidget, https, record, error, search, format, task, utilities) {
        ...
        function onRequest(context) {
            log.audit('Generacion de Archivo De Pagos Santander', 'INICIO - Metodo: ' + context.request.method);

            try {
                var esOneworld = utilities.l598esOneworld();

                var form = serverWidget.createForm({
                    title: 'Generacion de Archivo De Pagos Banco Santander'
                });

                form.clientScriptModulePath = './L598 - Generacion Archivo De Pagos Santander (Cliente).js'
                ...
                var bancoemisor = form.addField({
                    id: 'bancoemisor',
                    type: serverWidget.FieldType.SELECT,
                    label: 'Banco Emisor:',
                    source: 'customrecord_l598_bancos',
                    container: 'filtros'
                });
                ...
                if(!utilities.isEmpty(context.request.parameters.bancoemisor)){
                    bancoemisor.defaultValue = context.request.parameters.bancoemisor;
                }

                //INICIO - CAMPOS OBLIGATORIOS
                fechaDesde.isMandatory = true;
                fechaHasta.isMandatory = true;
                bancoemisor.isMandatory = true;
                ...
                if (context.request.method === 'GET') {
                    ...
                    //log.debug('Generacion de Archivo De Pagos Santander', 'LINE 320. sAccion : ' + sAccion);

                    switch (sAccion) {
                        case 'GENERARTXT':
                            var mensaje = "Se proceso su solicitud. Recibira una notificacion al finalizar por email";
                            //log.debug('Generacion de Archivo De Pagos Santander', 'context.request : ' + context.request+'. sublist: '+sublist);
                            ...
                            if (!utilities.isEmpty(resultado) && resultado.error == true) {
                                ...
                                log.error('Generacion de Archivo De Pagos Santander', 'Error Consulta Pagos Pendientes - Error : ' + mensaje);
                            }
                            infoResultado.defaultValue = '<font color="red">' + mensaje + '</font>';
                            log.audit('Generacion de Archivo De Pagos Santander', 'FIN Proceso');
                            context.response.writePage(form);
                            break;
                ...
                log.error('Generacion de Archivo De Pagos Santander', 'Excepcion Proceso Generacion de Archivo De Pagos Santander - Excepcion : ' + excepcion.message);
            }
        }

        function generarTXTMapReduce(sublistFacturas,sublistCheques, request) {
            //log.audit('Generacion de Archivo De Pagos Santander', 'LINE 354. INICIO Consulta Pagos Pendientes. request.parameters.pagospendientesdata: '+request.parameters.pagospendientesdata+'. sublistFacturas: '+sublistFacturas+'. sublistCheques: '+sublistCheques);
            //log.debug('Generacion de Archivo De Pagos Santander', 'LINE 355. info a procesar:' + JSON.stringify(request.parameters));
            ...
            try {
                if (!utilities.isEmpty(request.parameters.pagospendientesdata) || (!utilities.isEmpty(request.parameters.chequespendientesdata))) {
                    ...
                    var codigoBanco = request.parameters.bancoemisor;
                    //log.debug('Generacion de Archivo De Pagos Santander', 'LINE 371. codigoBanco: '+codigoBanco+'. pagosProcesados: '+pagosProcesados);
                    ...
                    if ((!utilities.isEmpty(sublista) && sublista.length > 0) || (!utilities.isEmpty(sublistaCheques) && sublistaCheques.length > 0)) {
                    	...
                    	//log.debug('Generacion de Archivo De Pagos Santander','LINE 464. idPagosProcesar: '+JSON.stringify(idPagosProcesar));
                        ...
                        if (respuesta.error == false) {
                            ...
                            parametros.custscript_l598_arch_pag_std_mr_id_pagos = idPagosProcesar.toString();
                            parametros.custscript_l598_arch_pag_std_mr_subsidia = subsidiaria;
                            parametros.custscript_l598_arch_pag_std_mr_cod_banc = codigoBanco;
                            log.debug('Generacion de Archivo De Pagos Santander', 'Generacion Pagos Realizados - Parametros: ' + JSON.stringify(parametros));
                            log.debug('Generacion de Archivo De Pagos Santander', 'INICIO llamada Script MAP/REDUCE');
                            respuesta = createAndSubmitMapReduceJob('customscript_l598_arch_pag_std_mr', parametros);
                            var mensajeEstado = "";
                            if (!utilities.isEmpty(respuesta) && !utilities.isEmpty(respuesta.estado))
                            {
                                mensajeEstado = respuesta.estado.status;
                            }
                            log.debug('Generacion de Archivo De Pagos Santander', 'MAP/REDUCE - Estado : ' + mensajeEstado);
                        }
                    } else {
                        respuesta.error = true;
                ...
                log.error('Generacion de Archivo De Pagos Santander', 'Consulta Pagos Pendientes - Excepcion Consultando Pagos Pendientes - Excepcion : ' + excepcion.message);
            }
            log.debug('Generacion de Archivo De Pagos Santander', 'FIN Consulta Pagos Pendientes');
            return respuesta;
        }

        function cargarPagos(sublistFacturas, sublistCheques, request, form) {

            log.debug('Generacion de Archivo De Pagos Santander', 'INICIO Consulta Pagos/Cheques Pendientes');
            var respuesta = new Object();
            respuesta.error = false;
            respuesta.mensaje = "";
            ...
            try {
                ...
                var pagosPendientes = search.load({
                    id: 'customsearch_l598_prev_pago_fact_santand'
                });

                var chequesPendientes = search.load({
                    id: 'customsearch_l598_prev_cheques_santander'
                });
                //log.debug('Generacion de Archivo De Pagos Santander', 'request.parameters.fechadesde: '+request.parameters.fechadesde+'. request.parameters.fechahasta: '+request.parameters.fechahasta+'. esOneworld: '+esOneworld);
                ...
                if (!utilities.isEmpty(request.parameters.bancoemisor)) {
                    var bancoemisor = request.parameters.bancoemisor;
                    if (!utilities.isEmpty(bancoemisor) && bancoemisor.length > 0) {
                        var filtroBancoemisor = search.createFilter({
                            name: 'custbody_l598_banco_emisor',
                            operator: search.Operator.IS,
                            values: bancoemisor
                        });
                
                        pagosPendientes.filters.push(filtroBancoemisor);
                        chequesPendientes.filters.push(filtroBancoemisor);
                ...
                //log.debug('Generacion de Archivo De Pagos Santander', 'LINE 607. INICIO Consulta Busqueda Pagos Pendientes');
                ...
                if (!utilities.isEmpty(completeResultSetPagosPend)) {
                    ...
                    log.debug('Generacion de Archivo De Pagos Santander', 'Cantidad de Pagos Pendientes: ' + completeResultSetPagosPend.length);
                    var idUnico = 0;
                    ...
                    log.audit('Generacion de Archivo De Pagos Santander', 'FIN Consulta Busqueda Pagos Realizados - No se encontraron Pagos Realizados');
                }
				//FIN - SI SE HALLO INFORMACION de Pagos de Facturas (VENDORPAYMENT) SE LLENA LA SUBLISTA

                log.debug("completeResultSetCheques",completeResultSetCheques);
                ...
                if (!utilities.isEmpty(completeResultSetCheques)) {
                    log.debug('Generacion de Archivo De Pagos Santander', 'Cantidad de Cheques Pendientes: ' + completeResultSetCheques.length);
                    var idUnico = 0;
                    var idUnicoAnterior = 0;
                    var idInternos = new Array();
                    var idInternos2 = new Array();
                    ...
                    while (!utilities.isEmpty(completeResultSetCheques) && completeResultSetCheques.length > 0 && i < completeResultSetCheques.length) {
                        ...
                        log.debug("beneficiario",beneficiario);
                        log.debug("ruc_proveedor",ruc_proveedor);
                        log.debug("fecha_emision",fecha_emision);
                        log.debug("importe",importe);
                        log.debug("tranId",tranId);
                        log.debug("procesado",procesado);
                        log.debug("moneda",moneda);
                        log.debug("banco",banco);
                        log.debug("fecha_pago",fecha_pago);
				...
				else
				{
                    ...
                    //log.audit('Generacion de Archivo De Pagos Santander', 'FIN Consulta Busqueda Pagos Realizados - No se encontraron Cheques Realizados');
                }
                ...
                respuesta.error = true;
                respuesta.mensaje = "Excepcion Consultando Pagos Pendientes - Excepcion : " + excepcion.message;
                log.error('Generacion de Archivo De Pagos Santander', 'Consulta Busqueda Pagos Realizados - Excepcion Consultando Pagos - Excepcion : ' + excepcion.message);
            }

            log.debug('Generacion de Archivo De Pagos Santander', 'FIN Consulta Pagos/Cheques Pendientes');
            return respuesta;
        }

        function createAndSubmitMapReduceJob(idScript, parametros) {
            log.debug('Generacion de Archivo De Pagos Santander', 'INICIO Invocacion Script MAP/REDUCE');
            var respuesta = new Object();
            respuesta.error = false;
            respuesta.mensaje = "";
            respuesta.estado = "";
            try {
                ...
                log.error('Generacion de Archivo De Pagos Santander', 'Generacion Pagos Realizados - Excepcion Invocando A Script MAP/REDUCE - Excepcion : ' + excepcion.message);
            }
            log.debug('Generacion de Archivo De Pagos Santander', 'FIN Invocacion Script MAP/REDUCE');
            return respuesta;
        }
